...
modul(dd1).
bezeichnung(dd1,'Digitale DatenÂ¸bertragung 1').
voraussetzung(sisy1,dd1).
ectsPunkte(dd1,4).
semester(dd1,5).

modul(dd2).
bezeichnung(dd2,'Digitale DatenÂ¸bertragung 2').
voraussetzung(dd1,dd2).
ectsPunkte(dd2,4).
semester(dd2,6).

modul(ood).
bezeichnung(ood,'Objektorienterte Analyse und Design fÂ¸r Informatiker').
...
modul(jff).
bezeichnung(jff,'Java fÂ¸r Fortgeschrittene').
...
%Aufgabe 3
% M Modul
% L Liste der wÃ¤hlbaren Module
waehlbarList(L,M) :- setof(X,waehlbar(X,M),L).

%Aufgabe 4
% L liste der gewaelten Module
% S Summe der ETCS Punkte
punkte([],S) :- S is 0.
punkte([L|LS],S) :- ectsPunkte(L,V),punkte(LS,REST),S is V + REST.
